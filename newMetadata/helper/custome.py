...
getentities = spacy.load('D:/FastAPI/metadata/helper/CustomRegexModelNER')

def get_entities(column, df, nlp, sample_size=10):
    entities = []
    ...
    most_common_entities = Counter(entities).most_common(1)[0][0]
...
def classify_location(df, column):
    ...
    return is_city, is_state, is_country
...
def is_address_line_column(column, df, nlp):
    ...
    # print(f"Results for column '{column}': {results}")
...
def detect_column_encoding(df, column, sample_size=10):
    ...
    for value in sample_values:
        raw_data = str(value).encode()  # Convert string to bytes
...
def is_valid_pincode(df, country, column, sample_size=10):
    ...
    return Counter(picodelist)


import phonenumbers
from collections import Counter

def is_phone(df, column, country="IN", sample_size=10):
    sample = df[column].dropna().sample(n=min(sample_size, len(df[column])), random_state=42)
    phonelist = []
